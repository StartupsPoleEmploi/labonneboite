...
def update_percentages():
    ...
    df_new_tension = pd.read_csv(
        "common/data/new_metiers_tension_2020.csv", sep=';')
    df_new_tension.rename(columns={'Métier': 'fap',
                                   'Difficultés à recruter': 'tension_pct_new'
                                   },
                          inplace=True)

    df_new_tension['tension_pct_new'] = df_new_tension.apply(
        lambda row: format_tension(row),
        axis=1
    )
    ...
    df_merge.rename(columns={'tension_pct_new': 'tension_pct'},
                    inplace=True)

    # Save new dataset to csv
    df_merge.to_csv('common/data/metiers_tension.csv',
                    encoding='utf-8',
                    sep=',',
                    index=False)
