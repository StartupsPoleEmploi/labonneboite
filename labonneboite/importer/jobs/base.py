...
class Job(object):
    ...
    def check_runnable(self):
        ...
        if not os.path.realpath(self.input_filename) in [os.path.realpath(f) for f in files]:
            msg = "File %s does not exist: not found in list %s" % (self.input_filename, ",".join(files))
            raise IOError(msg)
    ...
    def record_task(self):
        task = ImportTask(
            filename=os.path.basename(self.input_filename), state=ImportTask.FILE_READ, import_type=self.import_type
        )
        task.save()
