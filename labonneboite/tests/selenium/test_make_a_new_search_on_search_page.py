...
class TestMakeANewSearchOnSearchPage(LbbSeleniumTestCase):
    def setUp(self):
        super().setUp()
        url = self.url_for(
            "search.entreprises", l="Metz+57050", occupation="comptabilite", tr="car", lat="49.119146", lon="6.176026"
        )
        self.driver.get(url)
    ...
    def test_make_a_new_search_changing_location(self):
        ...
        WebDriverWait(self.driver, 60).until(EC.visibility_of_element_located((By.CSS_SELECTOR, "h1.lbb-result-info")))
    ...
    def test_make_a_new_search_using_isochronous_features(self):
        ...
        # Click on "min"
        self.driver.find_element_by_css_selector('.switch-element[data-switch-value="duration"]').click()

        # Travel modes should be visible now
        self.driver.find_element_by_css_selector('.travelmode-choices a.visible[data-travelmode="car"]').click()
