...
from labonneboite.common.maps import constants, exceptions
...
class IgnVendorIsochroneTest(AppTest):
    def test_valid_response_positive_lat_lon(self):
        response_content = open(os.path.join(os.path.dirname(__file__), "fixtures", "ign_metz_isochrone.json")).read()
        response = mock.Mock(status_code=200, json=mock.Mock(return_value=json.loads(response_content)))
    ...
    def test_valid_response_negative_latitude(self):
        response_content = open(
            os.path.join(os.path.dirname(__file__), "fixtures", "ign_saint_paul_la_reunion_isochrone.json")
        ).read()
        response = mock.Mock(status_code=200, json=mock.Mock(return_value=json.loads(response_content)))
        requests_get = mock.Mock(return_value=response)
    ...
    def test_valid_response_negative_longitude(self):
        response_content = open(
            os.path.join(os.path.dirname(__file__), "fixtures", "ign_la_rochelle_isochrone.json")
        ).read()
        response = mock.Mock(status_code=200, json=mock.Mock(return_value=json.loads(response_content)))
        requests_get = mock.Mock(return_value=response)
    ...
    def test_ign_authentication_error(self):
        response = mock.Mock(status_code=403, content="Invalid key")
        requests_get = mock.Mock(return_value=response)

        with self.test_request_context():
            with mock.patch.object(ign.requests, "get", requests_get):
                self.assertRaises(
                    exceptions.BackendUnreachable, ign.isochrone, places.metz, constants.ISOCHRONE_DURATIONS_MINUTES[0]
                )
...
class IgnVendorJourneyTest(unittest.TestCase):
    def mock_request_get(self):
        response_content = open(
            os.path.join(os.path.dirname(__file__), "fixtures", "ign_directions_metz_oxycoupure.json")
        ).read()
        response = mock.Mock(status_code=200, json=mock.Mock(return_value=json.loads(response_content)))
        requests_get = mock.Mock(return_value=response)
        return requests_get
