...
class NavitiaVendorTests(unittest.TestCase):
    ...
    def test_isochrone_valid_response(self, mock_coverage):
        response_content = open(
            os.path.join(os.path.dirname(__file__), "fixtures", "navitia_metz_isochrone.json")
        ).read()
    ...
    def test_no_solution_for_this_journey(self):
        data = {
            "context": {"current_datetime": "20190506T224043", "timezone": "Europe/Paris"},
            "disruptions": [],
            "error": {"id": "no_solution", "message": "no solution found for this journey"},
            "exceptions": [],
            "feed_publishers": [],
            "links": [],
            "notes": [],
