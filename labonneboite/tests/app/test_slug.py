...
class SlugDetailsTest(AppTest):
    ...
    def test_error_if_invalid_city_slugs(self):
        with self.test_request_context():
            rv = self.app.get("{}?city-slug={}".format(url_for("search.city_slug_details"), "INVALID_SLUG"))
            self.assertEqual(rv.status_code, 400)
    ...
    def test_ok_job_slug(self):
        with self.test_request_context():
            rv = self.app.get(
                "{}?job-slug={}".format(url_for("search.job_slug_details"), "abattage-et-decoupe-des-viandes")
            )
    ...
    def test_empty_list_if_invalid_slugs(self):
        with self.test_request_context():
            rv = self.app.get(
                "{}?job-slug={}".format(url_for("search.job_slug_details"), "INVALID_SLUG_1,INVALID_SLUG_2")
            )
            self.assertEqual(rv.status_code, 200)
            data = json.loads(rv.data.decode())
    ...
    def test_ok_city_slug(self):
        with self.test_request_context():
            rv = self.app.get("{}?city-slug={}".format(url_for("search.city_slug_details"), "nantes-44000"))
            self.assertEqual(rv.status_code, 200)
            data = json.loads(rv.data.decode())
...
class CityCodeDetailsTest(AppTest):
    def test_ok_city_code(self):
        with self.test_request_context():
            rv = self.app.get("{}?city-code={}".format(url_for("search.city_code_details"), "44109"))
            self.assertEqual(rv.status_code, 200)
            data = json.loads(rv.data.decode())
            self.assertEqual(data["city"]["name"], "Nantes")
    ...
    def test_error_if_invalid_city_code(self):
        with self.test_request_context():
            rv = self.app.get("{}?city-code={}".format(url_for("search.city_code_details"), "INVALID_CODE"))
            self.assertEqual(rv.status_code, 400)
