...
class IsochroneTest(AppTest):
    def test_valid_arguments(self):
        with self.test_request_context():
            rv = self.app.get(
                ...
                + "?zipcode={}&dur={}&tr={}".format(
                    "75000", constants.ISOCHRONE_DURATIONS_MINUTES[0], constants.DEFAULT_TRAVEL_MODE
                )
            )
    ...
    def test_invalid_zipcode(self):
        with self.test_request_context():
            rv = self.app.get(
                url_for("maps.isochrone")
                + "?zipcode={}&dur={}&tr={}".format(
                    "195000", constants.ISOCHRONE_DURATIONS_MINUTES[0], constants.DEFAULT_TRAVEL_MODE
                )
            )
    ...
    def test_invalid_duration(self):
        with self.test_request_context():
            rv = self.app.get(
                url_for("maps.isochrone")
                + "?zipcode={}&dur={}&tr={}".format("75000", 10000, constants.DEFAULT_TRAVEL_MODE)
            )
            self.assertEqual(400, rv.status_code)
    ...
    def test_non_integer_duration(self):
        with self.test_request_context():
            rv = self.app.get(
                url_for("maps.isochrone")
                + "?zipcode={}&dur={}&tr={}".format("75000", "plonk", constants.DEFAULT_TRAVEL_MODE)
            )
            self.assertEqual(400, rv.status_code)
    ...
    def test_invalid_travel_mode(self):
        with self.test_request_context():
            rv = self.app.get(
                url_for("maps.isochrone")
                + "?zipcode={}&dur={}&tr={}".format("75000", constants.ISOCHRONE_DURATIONS_MINUTES[0], "hoverboard")
            )
            self.assertEqual(400, rv.status_code)
    ...
    def test_missing_argument(self):
        with self.test_request_context():
            rv = self.app.get(
                url_for("maps.isochrone")
                + "?zipcode={}&dur={}".format("75000", constants.ISOCHRONE_DURATIONS_MINUTES[0])
            )
