...
from labonneboite.web.jepostule import utils, views


class JePostuleTests(TestCase):
    def test_get_token(self):
        mock_response = mock.Mock(
            status_code=200, json=mock.Mock(return_value={"token": "apptoken", "timestamp": 123})
        )
    ...
    def test_get_token_401(self):
        mock_response = mock.Mock(status_code=401, content="Error message", json=mock.Mock(side_effect=ValueError))
        with mock.patch.object(views.requests, "post", return_value=mock_response) as mock_post:
