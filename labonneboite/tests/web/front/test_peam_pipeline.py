...
class AuthPipelineTest(DatabaseTest):
    ...
    def test_run_pipeline_with_user_email_change(self):
        ...
        user.save()
        self.assertEqual(User.query.filter_by(external_id="peconnect-userid").first().email, "preexisting@email.com")
        result = self.run_pipeline(peam.PEAMOpenIdConnect)
        self.assertEqual(User.query.filter_by(external_id="peconnect-userid").first().email, "my@email.com")
