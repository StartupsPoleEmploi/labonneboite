...
class ApiGenericTest(ApiBaseTest):
    def test_happy_path(self):
        ...
        companies, _, _ = fetch_offices(
            naf_codes, [rome_code], latitude, longitude, distance, hiring_type=hiring_type_util.DPAE
        )
...
class ApiSecurityTest(ApiBaseTest):
    ...
    def test_wrong_timestamp_format(self):
        with self.test_request_context():
            params = self.add_security_params(
                {"commune_id": self.positions["caen"]["commune_id"], "rome_codes": "D1405", "user": "labonneboite"}
            )
    ...
    def test_expired_timestamp(self):
        with self.test_request_context():
            ...
            params["timestamp"] = (datetime.datetime.now() - datetime.timedelta(minutes=20)).strftime(
                "%Y-%m-%dT%H:%M:%S"
            )
            rv = self.app.get(self.url_for("api.company_list", **params))
            self.assertEqual(rv.status_code, 400)
    ...
    def test_missing_user_param(self):
        with self.test_request_context():
            params = self.add_security_params(
                {"commune_id": self.positions["caen"]["commune_id"], "rome_codes": "D1405,M1801"}
            )
            rv = self.app.get(self.url_for("api.company_list", **params))
            self.assertEqual(rv.status_code, 400)
    ...
    def test_unknown_user(self):
        with self.test_request_context():
            params = self.add_security_params(
                {"commune_id": self.positions["caen"]["commune_id"], "rome_codes": "D1405,M1801", "user": "unknown"}
            )
            rv = self.app.get(self.url_for("api.company_list", **params))
            self.assertEqual(rv.status_code, 400)
...
class ApiCompanyListTest(ApiBaseTest):
    ...
    def test_missing_communeid_or_latitudelongitude(self):
        with self.test_request_context():
            params = self.add_security_params({"rome_codes": "D1405", "user": "labonneboite"})
            rv = self.app.get(self.url_for("api.company_list", **params))
            self.assertEqual(rv.status_code, 400)
            self.assertEqual(
                rv.data, b"Invalid request argument: missing arguments: either commune_id or latitude and longitude"
            )

    def test_invalid_longitude_or_latitude(self):
        with self.test_request_context():
            params = self.add_security_params(
                {"rome_codes": "D1405", "user": "labonneboite", "latitude": "42", "longitude": "unparsable"}
            )
            rv = self.app.get(self.url_for("api.company_list", **params))
            self.assertEqual(rv.status_code, 400)
            self.assertEqual(rv.data, b"Invalid request argument: latitude and longitude must be float")
    ...
    def test_unknown_commune_id(self):
        with self.test_request_context():
            ...
            self.assertEqual(rv.status_code, 400)
            self.assertEqual(
                rv.data, b"Invalid request argument: could not resolve latitude and longitude from given commune_id"
            )
    ...
    def test_missing_rome_codes(self):
        with self.test_request_context():
            params = self.add_security_params(
                {"commune_id": self.positions["caen"]["commune_id"], "user": "labonneboite"}
            )
            rv = self.app.get(self.url_for("api.company_list", **params))
            self.assertEqual(rv.status_code, 400)
            self.assertEqual(
                rv.data, b"Invalid request argument: you must use rome_codes or rome_codes_keyword_search"
            )
    ...
    def test_rome_codes_search_by_keyword_normal_case(self):
        ...
        with self.test_request_context():
            ...
            params = self.add_security_params(
                {"commune_id": self.positions["caen"]["commune_id"], "user": "labonneboite", "rome_codes": rome_codes}
            )
            rv = self.app.get(self.url_for("api.company_list", **params))
            self.assertEqual(rv.status_code, 200)
    ...
    def test_rome_codes_search_by_keyword_when_no_match_found(self):
        with self.test_request_context():
            ...
            rv = self.app.get(self.url_for("api.company_list", **params))
            self.assertEqual(rv.status_code, 400)
            self.assertEqual(rv.data, b"Invalid request argument: No match found for rome_codes_keyword_search.")

    def test_rome_code_and_label_are_present_in_response(self):
        with self.test_request_context():
            rome = "D1501"
            params = self.add_security_params(
                {"commune_id": self.positions["caen"]["commune_id"], "user": "labonneboite", "rome_codes": rome}
            )
            rv = self.app.get(self.url_for("api.company_list", **params))
            data = json.loads(rv.data.decode())
    ...
    def test_error_when_latitude_or_longitude_are_empty(self):
        ...
        with self.test_request_context():
            params = self.add_security_params(
                {"latitude": "", "longitude": "", "rome_codes": "D1405", "distance": "10", "user": "labonneboite"}
            )
            rv = self.app.get(self.url_for("api.company_list", **params))
            self.assertEqual(rv.status_code, 400)
    ...
    def test_ok_when_latitude_or_longitude_equal_zero(self):
        ...
        with self.test_request_context():
            params = self.add_security_params(
                {"latitude": "0", "longitude": "0", "rome_codes": "D1405", "distance": "10", "user": "labonneboite"}
            )
            rv = self.app.get(self.url_for("api.company_list", **params))
            self.assertEqual(rv.status_code, 200)
    ...
    def test_query_by_departement(self):
        ...
        with self.test_request_context():
            params = self.add_security_params(
                {"departments": "14", "page": 1, "page_size": 2, "rome_codes": "D1405", "user": "labonneboite"}
            )
            rv = self.app.get(self.url_for("api.company_list", **params))
            self.assertEqual(rv.status_code, 200)
            data = json.loads(rv.data.decode())
    ...
    def test_multi_romes_search_is_supported_for_both_sort_by_score_and_by_distance(self):
        with self.test_request_context():
            ...
            params = self.add_security_params(
                {"commune_id": self.positions["caen"]["commune_id"], "rome_codes": "M1801", "user": "labonneboite"}
            )
            rv = self.app.get(self.url_for("api.company_list", **params))
            self.assertEqual(rv.status_code, 200)
            data = json.loads(rv.data.decode())
            ...
            self.assertEqual(set([c["siret"] for c in data["companies"]]), set(["00000000000004", "00000000000005"]))
    ...
    def test_same_rome_with_no_naf_filters(self):
        with self.test_request_context():
            ...
            params = self.add_security_params(
                {"commune_id": self.positions["metz"]["commune_id"], "rome_codes": "D1508", "user": "labonneboite"}
            )
            rv = self.app.get(self.url_for("api.company_list", **params))
            self.assertEqual(rv.status_code, 200)
            data = json.loads(rv.data.decode())
    ...
    def test_flag_alternance(self):
        with self.test_request_context():
            params = self.add_security_params(
                {"commune_id": self.positions["poitiers"]["commune_id"], "rome_codes": "B1603", "user": "labonneboite"}
            )
            rv = self.app.get(self.url_for("api.company_list", **params))
            self.assertEqual(rv.status_code, 200)
            data = json.loads(rv.data.decode())
    ...
    def test_flag_pmsmp_filter_incorrect_value(self):
        with self.test_request_context():
            ...
            with mock.patch.object(settings, "API_INTERNAL_CONSUMERS", ["labonneboite"]):
                ...
                rv = self.app.get(self.url_for("api.company_list", **params))
                self.assertEqual(rv.status_code, 400)
                self.assertIn("Invalid request argument: flag_pmsmp must be boolean (0 or 1)", rv.data.decode())
    ...
    def test_flag_pmsmp_filter_normal_behavior(self):
        with self.test_request_context():
            ...
            params = self.add_security_params(
                {"commune_id": self.positions["paris"]["commune_id"], "rome_codes": "N1202", "user": "labonneboite"}
            )
            rv = self.app.get(self.url_for("api.company_list", **params))
            self.assertEqual(rv.status_code, 200)
            data = json.loads(rv.data.decode())
    ...
    def test_filters_in_api_response(self):
        with self.test_request_context():
            params = self.add_security_params(
                {"commune_id": self.positions["toulon"]["commune_id"], "rome_codes": "N4403", "user": "labonneboite"}
            )
...
class ApiOffersOfficesListTest(ApiBaseTest):
    ...
    def test_happy_path(self):
        with self.test_request_context():
            ...
            with mock.patch(
                "labonneboite.common.esd.get_response", return_value=self.get_fixture("esd-offres-happypath.json")
            ) as mock_response:
