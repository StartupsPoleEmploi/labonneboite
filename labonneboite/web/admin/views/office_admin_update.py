...
from labonneboite.common import mapping as mapping_util, models
from labonneboite.common.siret import is_siret
from labonneboite.conf import settings
from labonneboite.importer.settings import (
    SCORE_ALTERNANCE_REDUCING_MINIMUM_THRESHOLD,
    SCORE_REDUCING_MINIMUM_THRESHOLD,
)
from labonneboite.web.admin.forms import nospace_filter, phone_validator, strip_filter
from labonneboite.web.admin.utils import AdminModelViewMixin, datetime_format
...
HIDE_CHECKBOXES = {"new_email": "remove_email", "new_phone": "remove_phone", "new_website": "remove_website"}
...
class OfficeAdminUpdateModelView(AdminModelViewMixin, ModelView):
    ...
    column_list = ["sirets", "name", "reason", "date_created", "date_updated"]
    ...
    form_args = {
        "sirets": {"filters": [strip_filter, nospace_filter]},
        "name": {"filters": [strip_filter]},
        "romes_to_boost": {"filters": [strip_filter]},
        "romes_to_remove": {"filters": [strip_filter]},
        "romes_alternance_to_boost": {"filters": [strip_filter]},
        "romes_alternance_to_remove": {"filters": [strip_filter]},
        "nafs_to_add": {"filters": [strip_filter]},
        "new_company_name": {"filters": [strip_filter], "validators": [validators.optional()]},
        "new_office_name": {"filters": [strip_filter], "validators": [validators.optional()]},
        "new_email": {"validators": [validators.optional(), validators.Email()]},
        "email_alternance": {"validators": [validators.optional(), validators.Email()]},
        "phone_alternance": {"validators": [validators.optional(), phone_validator]},
        "website_alternance": {"validators": [validators.optional(), validators.URL()]},
        "new_phone": {
            "filters": [strip_filter, nospace_filter],
            "validators": [validators.optional(), phone_validator],
        },
        "new_website": {"filters": [strip_filter], "validators": [validators.optional(), validators.URL()]},
        "social_network": {"validators": [validators.optional(), validators.URL()]},
        "requested_by_email": {"validators": [validators.optional(), validators.Email()]},
        "requested_by_first_name": {"filters": [strip_filter]},
        "requested_by_last_name": {"filters": [strip_filter]},
        "requested_by_phone": {
            "filters": [strip_filter, nospace_filter],
            "validators": [validators.optional(), phone_validator],
        },
        "recruiter_uid": {"filters": [strip_filter, nospace_filter]},
        "reason": {"filters": [strip_filter]},
    }
    ...
    def _handle_view(self, name, **kwargs):
        ...
        if name == "create_view":
            recruiter_message = self.get_recruiter_message(
                request.args.get("recruiter_message_type"), request.args.get("recruiter_message_id")
            )
    ...
    def prefill_form(self, form, form_id=None):
        ...
        self.handle_diff(
            "requested_by_first_name", recruiter_message.requested_by_first_name, office_admin_update, form
        )
        ...
        if recruiter_message.name == models.UpdateCoordinatesRecruiterMessage.name:
            ...
            self.handle_diff(
                "contact_mode", CONTACT_MODES.get(recruiter_message.contact_mode, ""), office_admin_update, form
            )
    ...
    def validate_form(self, form):
        ...
        for siret in sirets:
            ...
            if office_update_conflict.count() > 0:
                ...
                """.format(
                    siret, office_update_conflict[0].id
                )
                flash(message, "error")
                return False
