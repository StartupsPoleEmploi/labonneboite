...
from labonneboite.web.admin.utils import AdminModelViewMixin, SelectForChoiceTypeField, datetime_format
...
class OfficeAdminAddModelView(AdminModelViewMixin, ModelView):
    ...
    column_list = ["siret", "company_name", "reason", "date_created", "date_updated"]
    ...
    column_formatters = {"date_created": datetime_format, "date_updated": datetime_format}
    ...
    form_overrides = {"headcount": SelectForChoiceTypeField}

    form_args = {
        "siret": {"filters": [strip_filter, nospace_filter], "validators": [siret_validator]},
        "company_name": {"filters": [strip_filter]},
        "office_name": {"filters": [strip_filter]},
        "naf": {"filters": [strip_filter, nospace_filter]},
        "street_number": {"filters": [strip_filter, nospace_filter]},
        "street_name": {"filters": [strip_filter]},
        "zipcode": {"filters": [strip_filter, nospace_filter], "validators": [zip_code_validator]},
        "city_code": {"filters": [strip_filter, nospace_filter], "validators": [code_commune_validator]},
        "departement": {"filters": [strip_filter, nospace_filter]},
        "email": {"validators": [validators.optional(), validators.Email()]},
        "tel": {"filters": [strip_filter, nospace_filter], "validators": [validators.optional(), phone_validator]},
        "website": {"filters": [strip_filter], "validators": [validators.optional(), validators.URL()]},
        "headcount": {"choices": settings.HEADCOUNT_INSEE_CHOICES},
        "score": {"validators": [validators.NumberRange(min=0, max=100)]},
        "score_alternance": {"validators": [validators.NumberRange(min=0, max=100)]},
        "reason": {"filters": [strip_filter]},
        "x": {"filters": [strip_filter, nospace_filter], "validators": [validators.required()]},
        "y": {"filters": [strip_filter, nospace_filter], "validators": [validators.required()]},
    }
