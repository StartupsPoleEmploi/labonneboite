...
class OfficeAdminExtraGeoLocationModelView(AdminModelViewMixin, ModelView):
    ...
    column_list = ["siret", "codes", "reason", "date_created", "date_end"]
    ...
    column_descriptions = {"codes": "Veuillez entrer un département ou un code commune (INSEE) par ligne."}

    # `geolocations` is not included, it will be populated based on the `codes` content.
    form_columns = ["siret", "codes", "date_end", "reason"]

    form_args = {"codes": {"filters": [strip_filter]}, "reason": {"filters": [strip_filter]}}
