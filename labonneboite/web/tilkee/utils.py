...
ALLOWED_FILE_EXTENSIONS = (".doc", ".docx", ".jpeg", ".jpg", ".mov", ".mp4", ".pdf", ".png")
...
def process(files, company, user):
    ...
    documents = [
        {"name": fobj.filename, "s3_url": s3_url, "signable": False, "downloadable": True}
        for fobj, s3_url in zip(files, document_s3_urls)
    ]
    response = post(
        "/v2/wrapper/token_from_files",
        json={
            "project": {"name": "{} / {}".format(company.name, company.siret)},
            "person": {
...
def upload_file(fobj):
    ...
    try:
        response = requests.post(
            upload_url, data=data, files={"file": fobj}, headers=headers, timeout=AWS_HTTP_TIMEOUT_SECONDS
        )
    except requests.ReadTimeout:
        fobj.seek(0, 2)  # seek to end of file
        file_size_megabytes = fobj.tell() / (1024 * 1024.0)
...
def request(endpoint, method, **kwargs):
    ...
    """
    func = {"GET": requests.get, "POST": requests.post}[method]
    ...
    try:
        response = func(
            url, headers=headers, verify=settings.TILKEE_VERIFY_SSL, timeout=TILKEE_HTTP_TIMEOUT_SECONDS, **kwargs
        )
    except requests.ReadTimeout:
        message = "HTTP timeout url={url} method={method}".format(url=url, method=method)
        raise TilkeeError(message)
