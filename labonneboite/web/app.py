...
# labonneboite.
from labonneboite.common import hotjar, mailjet, pro
...
def create_app():
    ...
    assets.register(
        "recruiter_form",
        Bundle("js/recruiter-forms.js", filters="jsmin", output="gen/packed.recruiter_form.%(version)s.js"),
    )
...
def log_extra_context():
    extra = (("ip", request.remote_addr), ("method", request.method), ("path", request.full_path))
...
def social_auth_error(error):
    ...
    if (
        isinstance(error, social_exceptions.AuthFailed)
