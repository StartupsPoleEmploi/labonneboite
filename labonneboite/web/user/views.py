...
from labonneboite.common import activity, pdf as pdf_util, pro, util
from labonneboite.common.database import db_session
from labonneboite.common.models import Office, User, UserFavoriteOffice, get_user_social_auth
from labonneboite.common.pagination import FAVORITES_PER_PAGE, Pagination
...
def personal_data_as_csv():
    ...
    values = [current_user.first_name, current_user.last_name, current_user.email]
    content_row = ";".join(values)
    csv_text = "%s\r\n%s" % (header_row, content_row)

    return make_csv_response(csv_text=csv_text, attachment_name="mes_données_personnelles.csv")
...
def favorites_list_as_csv():
    ...
    return make_csv_response(
        csv_text=UserFavoriteOffice.user_favs_as_csv(current_user), attachment_name="mes_favoris.csv"
    )
...
def favorites_list():
    ...
    context = {"favorites": favorites, "pagination": pagination, "show_favorites": True}
