...
def create_visible_market_fetcher(request_args):
    ...
    return offers.VisibleMarketFetcher(
        romes=romes, commune_id=commune_id, distance=distance, hiring_type=hiring_type, page_size=page_size
    )
...
def create_hidden_market_fetcher(location, departements, request_args):
    ...
    if sort != sorting.SORT_FILTER_SCORE and location is None:
        raise InvalidFetcherArgument(
            "sort. %s is the only possible value with departement search. Please remove sort or departments."
    ...
    if request_args.get("naf_codes"):
        ...
        if invalid_nafs:
            raise InvalidFetcherArgument(
                "NAF code(s): %s. Possible values : %s " % (" ".join(invalid_nafs), ", ".join(expected_naf_codes))
            )
    ...
    if "headcount" in request_args:
        ...
        if not headcount:
            raise InvalidFetcherArgument(
                "headcount. Possible values : %s" % ", ".join(sorted(HEADCOUNT_VALUES.keys()))
            )
