...
from flask import Blueprint, Markup, flash, redirect, render_template, request, session, url_for
...
ACTION_NAMES = ["update_coordinates", "update_jobs", "delete", "other"]
...
def change_info_or_apply_for_job(siret=None):
    ...
    return render_template(
        "contact_form/change_info_or_apply_for_job.html", use_lba_template=is_recruiter_from_lba(), siret=siret
    )
...
def update_coordinates_form():
    ...
    form.new_contact_mode.data = request.form.get(
        "new_contact_mode", CONTACT_MODES_LABEL_TO_FORM_VALUE.get(office.contact_mode, "")
    )
    ...
    if form.validate_on_submit():
        ...
        if form_valid:
            ...
            params.update({"custom_ga_pageview": "/recruteur/update_coordinates/success", "suggest_update_jobs": "1"})
...
def update_jobs_form():
    ...
    if form.validate_on_submit():
        ...
        params = request.args.to_dict()
        params.update({"custom_ga_pageview": "/recruteur/update_jobs/success", "suggest_update_coordinates": "1"})
        return redirect(url_for("contact_form.success", **params))
...
def delete_form():
    ...
    if form.validate_on_submit():
        ...
        params = request.args.to_dict()
        params.update({"custom_ga_pageview": "/recruteur/delete/success"})
        return redirect(url_for("contact_form.success", **params))
...
def other_form():
    ...
    if form.validate_on_submit():
        ...
        params = request.args.to_dict()
        params.update({"custom_ga_pageview": "/recruteur/other/success"})
        return redirect(url_for("contact_form.success", **params))
