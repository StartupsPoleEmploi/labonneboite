...
def get_token_data(code):
    token_url = (
        settings.PEAM_AUTH_RECRUITER_BASE_URL
        + "connexion/oauth2/access_token?"
        + urllib.parse.urlencode({"realm": "/employeur"})
    )
...
def peam_recruiter_request(url, method="GET", headers=None, body=None):
    user_request = urllib.request.Request(
        url, method=method, headers=headers, data=urllib.parse.urlencode(body).encode("utf-8") if body else None
    )
    ...
    if response.status >= 400:
        message = 'HTTP error url={} method={} status={} content="{}"'.format(
            url, method, response.status, response.content
        )
        raise PeamRecruiterError(message)
