...
def deduplicate_users():
    ...
    for user in auth.User.query.group_by("external_id").having(sa.func.count(auth.User.external_id) > 1):
        ...
        for duplicate_user in auth.User.query.filter(
            auth.User.external_id == user.external_id, auth.User.id != user.id
        ):
        ...
        print(
            "Removing {} duplicates for user #{} ({} favorite added to original user)".format(
                len(duplicate_user_ids), user.id, favorite_count
            )
        )
        # Remove duplicate social user
        db_session.query(auth.UserSocialAuth).filter(auth.UserSocialAuth.user_id.in_(duplicate_user_ids)).delete(
