...
def _get_score_from_hirings(hirings, as_float=False):
    ...
    if hirings <= settings.SCORE_50_HIRINGS:
        ...
        score = 80.0 + 20.0 / math.log10(settings.SCORE_100_HIRINGS) * math.log10(
            1 + hirings - settings.SCORE_80_HIRINGS
        )
    else:
...
def get_hirings_from_score(score):
    ...
    if score <= 50:
        ...
        hirings = (
            -1 + settings.SCORE_80_HIRINGS + 10.0 ** ((score - 80) / 20.0 * math.log10(settings.SCORE_100_HIRINGS))
        )
    else:
