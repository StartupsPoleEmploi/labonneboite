...
def isochrone(origin, duration):
    ...
    for polygon in polygons:
        ...
        isochrones.append([(float(lat), float(lon)) for lon, lat in lon_lat])
    return isochrones
...
def request_json_api(endpoint, params, timeout):
    ...
    auth = (
        HTTPBasicAuth(ign_credentials["username"], ign_credentials["password"])
        if "username" in ign_credentials
        else None
    )
    headers = {"Referer": ign_credentials["referer"]} if "referer" in ign_credentials else None
